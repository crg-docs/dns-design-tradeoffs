...
この章では、三つの時代を区別します。DNS以前にも、ホスト名とアドレスの対応を調べる仕組みはありました。1983年に提案され、1987年に現在の骨格が定まったDNSは、その対応情報を大規模に管理し、参照する方法を変えました。その後、同じ仕組みの上へ負荷分散や障害時の切り替えなどの用途が加わりました。後から普及した用途を、最初からの目的だったかのようには扱いません。

## 1. 名前は通信に使う情報そのものではない
...
そこで、人やアプリケーションが参照する名前と、通信に必要な情報を分けます。名前を使う側は名前を保持し、その名前に現在対応する情報を別に管理します。名前に対応する情報を後から変更できれば、名前を使う側の記録をすべて同時に書き換えずに済みます。ただし、変更済みの情報がいつ利用者から見えるかは、キャッシュなどにも左右されます。変更が即座に全利用者へ届くという意味ではありません。
...
利用者が名前を暗記している必要もありません。WebブラウザーのブックマークはURLを保持し、URLには接続先を示す名前を含められます。リンクを選ぶ場合も、アプリケーションが設定済みの接続先を使う場合も、利用者の代わりにソフトウェアが名前を保持しているだけです。利用者が文字列を意識しなくても、ソフトウェアが名前を使うなら、その名前から必要な情報を得る処理は残ります。

ここでURLとドメイン名を同じものとはみなしません。1994年のRFC 1738が定めたURLは、アクセス方法やホストなどを含み、ホスト名はその一部として現れます。一方、次節で扱うホスト名の資料は1973年のものです。したがって、DNSを「URLを覚えやすくするために作られた仕組み」とは説明できません。[RFC 1738: Uniform Resource Locators (URL)](https://www.rfc-editor.org/rfc/rfc1738.html)
...
1. 名前と、通信に必要な情報を結び付けること
2. その対応情報を、誰が正しい状態に保ち、利用者へ届けるか

名前解決は一つ目に答えますが、対応表を作っただけでは二つ目は終わりません。
...
DNSがこの段階で中心的に解こうとしたのは、名前を人に覚えさせる問題ではありません。**名前に対応する情報を、一つの組織と一つの一覧へ集めずに管理し、必要な情報を参照できるようにすること**でした。そのために、名前空間を階層化し、範囲ごとに管理責任を委ねる設計が採用されました。具体的な責任分割は第2章で扱います。
...
また、DNSの応答を変更するだけで、瞬時で確実な切り替えが成立するわけではありません。以前の応答がキャッシュに残る時間、障害を検出して情報を変更する外部の仕組み、複数の候補から接続先を選ぶ側の動作が関係します。負荷分散は第3章、障害時の切り替えは第4章、変更が見えるまでの時間は第5章で、それぞれの要件から評価します。
