...
階層化だけでは、更新権限は分かれません。DNSは、名前空間の一部に対する権威を別の管理主体へ移す**委譲**を組み合わせます。親側は、子側の全データを持つ代わりに、どの権威サーバーへ問い合わせを進めればよいかを示します。子側は委ねられた範囲のデータを管理します。中央がすべての末端データを更新する必要をなくしながら、親から子へたどれる関係を残す設計です。[RFC 1034: Domain Names—Concepts and Facilities](https://www.rfc-editor.org/rfc/rfc1034.html) 2.4節、4.2節

### ドメインとゾーンは同じ境界ではない

RFC 9499の現行用語では、**ドメイン**は名前空間中のあるノードと、その下にあるすべてのノードからなる部分木です。これに対して**ゾーン**は、ある権威サーバー群が同じ権威情報を提供する範囲です。部分木の途中を子へ委譲すると、委譲先の部分木は元のドメインに含まれたままでも、親のゾーンには含まれません。[RFC 9499: DNS Terminology](https://www.rfc-editor.org/rfc/rfc9499.html) 2節、7節

次の図では、名前空間上は `dev.example.jp` も `example.jp` ドメインの一部です。しかし、`dev.example.jp` を別の管理主体へ委譲したため、二つは別のゾーンです。親ゾーンは子ゾーンの `api` の値を管理せず、委譲先を示す情報を管理します。
...
この区別によって、一意性を保つ責任にも境界ができます。親は、自分の直下で同じ子の名前を二つの管理主体へ委譲しない責任を持ちます。委譲された側は、その部分木の中で名前を割り当てます。全体の一意性を捨てたのではなく、「どの範囲で誰が一意性を保つか」を階層に沿って分割したのです。

### 一括配布から必要な情報の参照へ

委譲されたデータは一つの表に集め直されません。リゾルバは親が示す委譲情報をたどり、必要な名前の権威情報を持つサーバーへ問い合わせます。これにより、ある組織の変更を、名前空間全体の利用者へ一覧として配る必要はなくなります。ただし、問い合わせ経路とキャッシュの詳しい動作は性能を担当する第3章で扱います。ここで重要なのは、データの所在だけでなく、データを見つけるための参照関係も分散したことです。
...
- [RFC 1034: Domain Names—Concepts and Facilities](https://www.rfc-editor.org/rfc/rfc1034.html) 2.1節、2.4節、4.2節
