...
このページは本書の本文ではない。第3章、第5章で使う図の下書きを置く作業用ページである。年表は第1章へ、階層的名前空間と委譲の図は第2章へ取り込み済みのため削除した。各章が図を取り込んだら、対応する節を削除する。全章完成時に空になっていることを #19 の通し確認で確かめる。
...
## 1. 反復問い合わせの経路（mermaid: sequenceDiagram）
